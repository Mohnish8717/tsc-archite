...
class AgentGraph:
    ...
    def add_edge(self, agent_id_0: str, agent_id_1: str):
        try:
            self.graph.add_edge(agent_id_0, agent_id_1)
        except ig.InternalError as e:
            logging.getLogger(__name__).warning(f"igraph InternalError adding edge {agent_id_0}->{agent_id_1}: {e}")
        except Exception as e:
            logging.getLogger(__name__).error(f"Unexpected error adding edge {agent_id_0}->{agent_id_1}: {e}")
